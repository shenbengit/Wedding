...
public class ComPereViewModel extends BaseViewModel<BusinessModel> {
    ...
    public ComPereViewModel(@NonNull Application application) {
    ...
    private void showPopup(List<RecyclerPopupBean> list) {
        mPopup.setNewData(BUSINESS_EMCEE, list);
    ...
    private void screenBusinessAreaList() {
        ...
        if (TextUtils.equals(areaString.get(), mAreaList.get(0).getOption())) {
            ...
            BusinessEmceeBean photographyBean;
            for (MultiItemEntity entity : mBusinessInfoList) {
                photographyBean = (BusinessEmceeBean) entity;
    ...
    private synchronized void getBusinessList(int page, int perPage) {
        mModel.getBusinessList(mLifecycleProvider, page, perPage, BUSINESS_EMCEE, 0, 0, "default",
                new Action() {
                    @Override
                    public void run() throws Exception {
